    ...
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node
      ...
      \path
        ...
        (04) edge [bend left=30]
            node [right] {$\mathtt{r^{mc91} := t}$} (05)

        (05) edge [bend left=30]
             node [below=8] {$\mathtt{r := r^{mc91}}$} (2);
  ...
  \resizebox{.9\textwidth}{!}{
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node
      ...
      \path
        ...
        (4) edge [bend left=30]
            node [below] {$\mathtt{r^{mc91} := t}$} (e)
        (e) edge [bend left=15]
            node [below=9] {$\mathtt{r := r^{mc91}}$} (02)
        ...
        (s') edge [bend right=30]
             ...
             node [right] {$\mathtt{assume\ not(m' > 100)}$} (2')
        (1') edge [bend right=30]
             node [left] {$\mathtt{r^{mc91'} := m' - 10}$} (e')
        (2') edge 
             node [right] {$\mathtt{s' := mc91(m'+11)}$} (3')
        (3') edge 
             node [right] {$\mathtt{t' := mc91(s')}$} (4')
        (4') edge [bend left=30]
            node [right] {$\mathtt{r^{mc91'} := t'}$} (e')
        (e') edge 
             node [above] {$\mathtt{s := r^{mc91'}}$} (3)
        ...
        (4'') edge [bend left=30]
              node [right] {$\mathtt{r^{mc91''} := t''}$} (e'')
        (e'') edge [bend left=50]
              node [below left] {$\mathtt{t := r^{mc91''}}$} (4)
        ;
    \end{tikzpicture}
    }
