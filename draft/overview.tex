...
\begin{algorithm}
  \KwIn{A program $P=\{\mbox{CFG }G_\fun{main}\}\cup\{\mbox{CFG } G_\fun{f} : \fun{f}\mbox{ is a function}\}$}
  ...
  \Repeat{$\textmd{complete?}$}
  {
    ...
    \Switch{\textsc{BasicAnalyzer} ($G^-_k$)}
    {
      \lCase{$\mathit{Pass(\Pi (G^-_k, \TT))}$:}
      {    
        S := UpdateSummary ($P$, $G^-_k$, $\Pi (G^-_k, \TT)$)\;
      }
      \lCase{$\mathit{Error}$:}
      {
        \Return $\mathit{Error}$\;
...
$G^-_k$ is recursion-free. \textsc{BasicAnalyzer} is able to
...
\begin{figure}
  \centering
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node
      ...
      \node[node] (00) at ( 0,  0) {\smallnode{$\ell_{s'}^{1,\fun{mc91}}$}};
      \node[node] (01) at (-1, -2) {$\ell_{1'}^1$};
      \node[node] (02) at ( 1, -0.8) {$\ell_{2'}^1$};
      \node[node] (03) at ( 1, -2) {$\ell_{3'}^1$};
      \node[node] (04) at ( 1, -3.2) {$\ell_{4'}^1$};
      \node[node] (05) at ( 0, -4) {\smallnode{$\ell_{e'}^{1,\fun{mc91}}$}};

      \path
        (0) edge 
            node [left] {$\mathtt{assume\ n >= 0}$} (1)
        (1) edge [bend left=15]
            node [above=2] {$\mathtt{m^1 := n}$} (00)
        (2) edge
            node [left] {$
              \begin{array}{l}
        ...
        (00) edge [bend right=30]
            node [left] {$\mathtt{assume\ m^1 > 100}$} (01)
            edge [bend left=30]
            node [right] {$\mathtt{assume\ not(m^1 > 100)}$} (02)
        ...
        (04) edge [bend left=30]
            node [right] {$\mathtt{r_{mc91}^1 := t^1}$} (05)

        (05) edge [bend left=30]
             node [below=8] {$\mathtt{r := r_{mc91}^1}$} (2);
    ...
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node
      ...
      \path
        ...
        (1) edge [bend right=30]
            node [left] {$\mathtt{r_{mc91} := m - 10}$} (5)
        (2) edge 
            node [right] {$
              \begin{array}{l}
              \mathtt{s := nondet;}\\
...
\begin{figure}
  ...
  \resizebox{.9\textwidth}{!}{
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node
      ...
      \node[node] (s) at (-1,  0) {\smallnode{$\ell_{s'}^{1,\fun{mc91}}$}};
      \node[node] (1) at (-2, -2) {$\ell_{1'}^1$};
      \node[node] (2) at ( 1, -0.8) {$\ell_{2'}^1$};
      \node[node] (3) at ( 1, -2) {$\ell_{3'}^1$};
      \node[node] (4) at ( 1, -3.2) {$\ell_{4'}^1$};
      \node[node] (e) at (-1, -4) {\smallnode{$\ell_{e'}^{1,\fun{mc91}}$}};

      \node[node] (s') at ( 5,  2.6) {$\ell_{s'}^2$};
      \node[node] (1') at ( 4,  0.6) {$\ell_{1'}^2$};
      \node[node] (2') at ( 6,  1.8) {$\ell_{2'}^2$};
      \node[node] (3') at ( 6,  0.6) {$\ell_{3'}^2$};
      \node[node] (4') at ( 6, -0.6) {$\ell_{4'}^2$};
      \node[node] (e') at ( 5, -1.4) {$\ell_{e'}^2$};

      \node[node] (s'') at ( 5, -2.6) {$\ell_{s'}^3$};
      \node[node] (1'') at ( 4, -4.6) {$\ell_{1'}^3$};
      \node[node] (2'') at ( 6, -3.4) {$\ell_{2'}^3$};
      \node[node] (3'') at ( 6, -4.6) {$\ell_{3'}^3$};
      \node[node] (4'') at ( 6, -5.8) {$\ell_{4'}^3$};
      \node[node] (e'') at ( 5, -6.6) {$\ell_{e'}^3$};

      \path
        (0s) edge 
            node [left] {$\mathtt{assume\ n >= 0}$} (01)
        (01) edge [bend left=15]
            node [above=2] {$\mathtt{m^1 := n}$} (s)
        (02) edge 
            node [left] {$
              \begin{array}{l}
                \mathtt{assert\ {[}r = 91\ or}\\
        ...
        (1) edge [bend right=30]
            node [above right] {$
              ...
              \end{array}$} (e)
        (2) edge [bend left=50]
            node [above left] {$\mathtt{m^2 := m^1 + 11}$} (s')
        (3) edge 
            node [below] {$\mathtt{m^3 := s^1}$} (s'')
        (4) edge [bend left=30]
            node [below] {$\mathtt{r_{mc91}^1 := t^1}$} (e)
        (e) edge [bend left=15]
            node [below=9] {$\mathtt{r := r_{mc91}^1}$} (02)
        ...
        (s') edge [bend right=30]
             node [left] {$
               ...
               \end{array}$} (1')
             edge [bend left=30]
             node [right] {$\mathtt{assume\ not(m^2 > 100)}$} (2')
        (1') edge [bend right=30]
             node [left] {$\mathtt{r_{mc91}^2 := m^2 - 10}$} (e')
        (2') edge 
             node [right] {$\mathtt{s^2 := mc91(m^2+11)}$} (3')
        (3') edge 
             node [right] {$\mathtt{t^2 := mc91(s^2)}$} (4')
        (4') edge [bend left=30]
            node [right] {$\mathtt{r_{mc91}^2 := t^2}$} (e')
        (e') edge 
             node [above] {$\mathtt{s^1 := r_{mc91}^2}$} (3)

        (s'') edge [bend right=30]
              node [left] {$\mathtt{assume\ m^3 > 100}$} (1'')
              edge [bend left=30]
              node [right] {$\mathtt{assume\ not(m^3 > 100)}$} (2'')
        (1'') edge [bend right=30]
              node [left] {$
                \begin{array}{c}
                \mathtt{r_{mc91}^3 :=}\\
                ...
                $} (e'')
        (2'') edge 
              node [right] {$\mathtt{s^3 := mc91(m^3+11)}$} (3'')
        (3'') edge 
              node [right] {$\mathtt{t^3 := mc91(s^3)}$} (4'')
        (4'') edge [bend left=50]
              node [right] {$\mathtt{r_{mc91}^3 := t^3}$} (e'')
        (e'') edge [bend left=50]
              node [below left] {$\mathtt{t^1 := r_{mc91}^3}$} (4)
        ;
    \end{tikzpicture}
    }
