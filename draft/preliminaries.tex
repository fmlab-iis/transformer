...
For a sequence of variables $\overline{x}$, we use $x_i$ to denote its $i$-th element. To simplify presentation, we assume the functions in the programs uses different sets variables and the values of formal parameters never change in a function. Notice that this will not affect the expressiveness of a CFG because one can still make copies of formal parameters by assignments and change the values of the copied versions. Also we assume that there exists no global variables because this can be simulated by allowing simultaneous assignment to return variables. 
...
\begin{figure}
  ...
  \begin{subfigure}[b]{.6\textwidth}
    \begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node
      distance=2cm,thick,node/.style={circle,draw}]
      \node[node] (0) at ( 0,  0) {$\ell'_s$}; %[label=above:$\mathtt{mc91(n)}$]
      \node[node] (1) at (-1, -2) {$\ell'_1$};
      \node[node] (2) at ( 1, -0.8) {$\ell'_2$};
      \node[node] (3) at ( 1, -2) {$\ell'_3$};
      \node[node] (4) at ( 1, -3.2) {$\ell'_4$};
      \node[node] (5) at ( 0, -4) {$\ell'_e$};
