...
\end{equation*}
If a computation does not terminate with a $\mathtt{return}$ command, its semantics is the same to a infinite computation without returning any value.
...
\begin{figure}
  ...
  \end{subfigure}
  \caption{McCarthy 91. The function $\mathtt{mc91}$ has formal parameter $m$ and return variable $r^\mathtt{mc91}$}
  \label{figure:mccarthy91}
\end{figure}

%\todo{definitions of accessible variables, program states}
Let $G = \langle V, E ,\textmd{cmd},\bar{u}, \bar{r},  \ell_s,  \ell_e\rangle$ be a CFG.
...
\begin{center}
  ...
  \AxiomC{$E \implies Q$}
  \LeftLabel{Assert}
  \BinaryInfC{$\assert{P} \mathtt{assert\ } E \assert{Q}$}
  \DisplayProof
\end{center}
Observe
